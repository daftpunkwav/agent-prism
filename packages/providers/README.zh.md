...
LLM provider 配置与模型构造。`provider-capability` 是无 SDK 的纯 seam，含 endpoint
...
| Package | 职责 | 挂接位置 |
|---|---|---|
| [`provider-capability/`](provider-capability/README.md) | Provider seam：`ProviderLookupAdapter`，实现 `contracts.ProviderLookup`；`EndpointCatalog`、`ProviderConfigStore`、config 解析、thinking budget | Seam，零 SDK 依赖 |
