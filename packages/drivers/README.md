...
Framework-adapter layer for execution semantics: a run column can execute on any supported framework. `driver-registry` owns the `DriverLookup` seam and shared run support; each backend leaf adapts exactly one framework and is registered explicitly at the single composition root (`apps/server/src/assemble.ts`). Heavy framework dependencies live only in backend leaves; the seam and the native backend stay LangChain-free.

## Subpackages

| Package | Role | Wired at |
|---|---|---|
| [`driver-registry/`](driver-registry/README.md) | Driver services: `DriverLookup` registry, best-effort registration helper, shared run support (event translation, capability banner, recursion cap) | Seam (zero backend dependencies) |
