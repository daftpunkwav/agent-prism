...
export function ExperimentPanel({ dimension, columnCount, onConfigChange }: ExperimentPanelProps) {
    ...
    fetchProvider()
      .then((cfg: ProviderConfig) => {
        ...
        const est = estimatePromptTokens(
          ...
          "Q: 现在几点？ → Thought: 需要当前时间 → Action: get_current_time"
        );
        setEstimatedTokens(est * columnCount);
      })
      .finally(() => setLoading(false));
  }, [columnCount, onConfigChange]);

  const onTemperatureChange = useCallback(async (value: number) => {
    if (!config) return;
    const next = { ...config, temperature: value };
    setConfig(next);
    try {
      await saveProvider({ ...next, api_key: "" });
      onConfigChange?.({
        model: next.model,
        temperature: next.temperature,
        contextWindow: next.context_window,
        maxInputTokens: next.max_input_tokens,
        maxOutputTokens: next.max_output_tokens,
      });
    } catch {
      // 静默
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          <div className="rounded border border-border bg-muted/30 p-2 text-center">
            <div className="text-[10px] text-muted-foreground">窗口</div>
            <div className="font-mono text-xs mt-0.5">
              {(config.context_window / 1000).toFixed(0)}k
            </div>
          </div>
          <div className="rounded border border-border bg-muted/30 p-2 text-center">
            <div className="text-[10px] text-muted-foreground">最大输入</div>
            <div className="font-mono text-xs mt-0.5">
              {(config.max_input_tokens / 1000).toFixed(0)}k
            </div>
          </div>
          <div className="rounded border border-border bg-muted/30 p-2 text-center">
            <div className="text-[10px] text-muted-foreground">最大输出</div>
            <div className="font-mono text-xs mt-0.5">
              {(config.max_output_tokens / 1000).toFixed(0)}k
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded border border-border bg-muted/20 p-3 space-y-2">
        <div className="flex items-center gap-1.5 cursor-pointer" onClick={() => setShowEstimate(!showEstimate)}>
        ...
        {showEstimate && (
          <div className="text-[11px] text-muted-foreground space-y-1 pt-1 border-t border-border">
            ...
            <p>用户 prompt 约 {Math.ceil(estimatePromptTokens("", "Q: 现在几点？") / columnCount)} tokens × {columnCount} 列 = ~{Math.ceil(estimatePromptTokens("", "Q: 现在几点？"))} tokens</p>
      ...
      <div className="rounded border border-border bg-card p-3">
        ...
        <p className="text-xs">
          {dimension === "framework" && "并排运行 LangChain / LangGraph，编排实现不同，其余配置保持一致"}
          {dimension === "prompt" && "同一编排，4 种 Prompt 策略对比：Zero-shot / Few-shot / CoT / Structured"}
          {dimension === "reasoning" && "4 种推理模式对比：ReAct / CoT+Tool / ToT / Reflexion"}
          {dimension === "context" && "4 种上下文策略对比：滑动窗口 / 摘要 / 向量 / 混合"}
          {dimension === "harness" && "4 种 Harness 级别对比：裸运行 / 验证 / 反思 / 自进化"}
        </p>
      </div>
    </div>
  );
